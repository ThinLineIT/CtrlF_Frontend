...
export default function MarkdownEditor(props) {
    ...
    const getUrl = async file => {
        if (file && file.size < 5000000) {
            ...
            const result = await axios({
                method: 'post',
                url: `${process.env.NODE_ENV === 'development' ? process.env.NEXT_PUBLIC_DEVELOP_API_BASE_URL : process.env.NEXT_PUBLIC_RELEASE_API_BASE_URL}actions/images/`,
                data: body,
            })
                .then(res => res)
                .catch(err => err);
    ...
    const imgAdding = url => {
        ...
        TextArea.value = origin.substring(0, sStart) + `![img](${url})` + origin.substring(sEnd, origin.length);
    ...
    let previewContents = !addNewPageContent ? (input == null ? content : input) : input;
    ...
    return (
        <form className={styles.editor_wrap}>
            ...
            <textarea type="text" placeholder="summary" onChange={onPageSummaryHandler} className={styles.users_summary} />
            <span as="h3" className={styles.detail_content}>
                <div className={styles.buttonsWrap}>
                    <span className={styles.editor_button}>
                        {WRITE_OR_PREVIEW.map((button, i) => (
                            <button key={i} className={styles.statusBtn} onClick={e => onChangeStatus(e, button)}>
                                {button}
                            </button>
                        ))}
                    </span>
                    <div className={styles.buttonsContainer}>
                        {btnsObj.allEditBtns.map((button, i) => (
                            <button key={i} className={styles.btn} onClick={e => addFuncButtons(e, button)}>
                                {button}
                            </button>
                        ))}
